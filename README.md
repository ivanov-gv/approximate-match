...
`NewMatcher` preprocesses the word list once. Call `Find` for each query — it returns matches sorted by score (1.0 = identical, 0 = no commonality).

The matcher handles case differences, spaces, diacritics (š→s, č→c, ž→z, ć→c, ö→o, ü→u, etc.), phonetic variants (sh/ch/ph, ije↔e, lj→l), and severe vowel confusion via a consonant-skeleton fallback.
